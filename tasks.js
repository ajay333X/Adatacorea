...
// Global variables for audio-plus.js
...
window.submitAndLoadNextTask = async function () {
  ...
  reader.onloadend = async () => {
    try {
      await callApi("submitAudioTask", {
        taskId: window.currentTask.taskId,
        durationSec: String(Math.floor((Date.now() - window.taskStartTime) / 1000)),
...
onAuthStateChanged(window.firebaseAuth, async (u) => { if(u) await callApi("getUserDashboardData"); });
